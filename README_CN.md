...
无需公网 IP，让家庭、公司、云端的 AI Agent 自动互联。

OpenAFP 是一个专门为 AI Agent（如 Claude Desktop、MCP Server、自动化工具）设计的连接层。它可以让位于家庭、公司、云服务器中的 Agent，在复杂网络环境下自动建立连接与发现彼此。

无需公网 IP。无需端口映射。无需复杂内网穿透配置。
